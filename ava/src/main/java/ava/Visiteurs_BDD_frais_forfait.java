...
public class Visiteurs_BDD_frais_forfait {
    ...
    public static void connexionALaBDD(int id_user) {
        ...
        try {
            ...
            rst = st.executeQuery(
                    "SELECT utilisateurs.nom_user, utilisateurs.prenom_user, frais_forfaitaire.cout_hotel, frais_forfaitaire.cout_repas, frais_forfaitaire.cout_kilometrage FROM remboursement JOIN frais_forfaitaire on frais_forfaitaire.id_ask = utilisateurs.id_user WHERE id_ask = "
                            + id_user);
            ...
            while (rst.next()) {
                ...
                if (accepter_remboursement != 0) {
                    System.out.println("[ Remboursement demandé par " + nom_user + " " + prenom_user
                            + " pour un montant de " + montant_remboursement + "€ pour la raison suivante: \""+ raison_remboursement +"\". Accepté par " + nom_user2 + " "
                            + prenom_user2 + " ]");
                }
            }
